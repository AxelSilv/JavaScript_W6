...
function years(from, to){const a=[];for(let i=from;i<=to;i++)a.push(String(i));return a;}
...
async function init(){
  try{
    if(!Object.keys(AREA_NAME_BY_CODE).length){ await loadAreas(); }
    const areaCode = sessionStorage.getItem("wk6_area_code") || "SSS";
    const data = await postForArea(areaCode);
    renderChart(data, areaCode);
  }catch(e){}
  if(!listenersBound){
    listenersBound = true;
    document.getElementById("submit-data").addEventListener("click", onFetchMunicipality);
    document.getElementById("add-data").addEventListener("click", onAddDataPoint);
  }
}
...
async function loadAreas(){
  ...
  AREA_NAME_BY_CODE = {};
  AREA_CODE_BY_NAME = {};
  codes.forEach((code,i)=>{ const name=names[i]; AREA_NAME_BY_CODE[code]=name; AREA_CODE_BY_NAME[name.toLowerCase()]=code; });
}
...
function renderChart(json, areaCode){
  sessionStorage.setItem("wk6_area_code", areaCode);
...
function drawChart(){
  ...
  chart = new frappe.Chart("#chart",{
    ...
    data:{ labels:currentLabels, datasets:[{ name:"Population", values:currentValues }] },
    type:"line",
    height:450,
    colors:["#eb5146"],
...
async function onFetchMunicipality(){
